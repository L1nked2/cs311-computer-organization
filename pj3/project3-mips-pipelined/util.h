...
typedef struct CPU_State_Struct {
  ...
  /***********************************/
  // PC
  unsigned char PC_SRC; /* Choose NPC */
  //To choose right PC
	uint32_t IF_PC;
	uint32_t JUMP_PC;
	uint32_t BRANCH_PC;
  unsigned char PC_WRITE; /* Check if PC is writable */
  // ID/EX
  unsigned char ID_EX_REG_DST; /* determines src of reg1, select RD if TRUE, select RT if FALSE*/
  unsigned char ID_EX_ALU_OP; 
  unsigned char ID_EX_ALU_SRC; /* determines src of reg2, select IMM if TRUE, select RT if FALSE*/
  unsigned char ID_EX_MEM_BRCH;
  unsigned char ID_EX_MEM_READ;
  unsigned char ID_EX_MEM_WRITE;
  unsigned char ID_EX_REG_WRITE;
  unsigned char ID_EX_MEM_TO_REG;
  // EX/MEM
  ...
  unsigned char ID_LOAD_USE_HAZARD;
